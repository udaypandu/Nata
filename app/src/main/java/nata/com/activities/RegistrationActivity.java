...
public class RegistrationActivity extends AppCompatActivity implements View.OnClickListener, IAsyncCaller {
    ...
    private TextView tv_already_have_an_act;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_registration_janasena);
        initUI();
    }
    ...
    private void initUI() {
        ...
        tv_already_have_an_act = (TextView) findViewById(R.id.tv_already_have_an_account_sign_in);
        ...
        edt_profession.setOnClickListener(this);
        edt_district.setOnClickListener(this);
        edt_mandal.setOnClickListener(this);
        edt_village.setOnClickListener(this);
        edt_select_id.setOnClickListener(this);
        edt_enter_id_proof.setOnClickListener(this);
        tv_already_have_an_act.setOnClickListener(this);

        intent = getIntent();
        if (intent.hasExtra(Constants.REGISTRATION_EMAIL)) {
            mEmailId = intent.getStringExtra(Constants.REGISTRATION_EMAIL);
            if (!Utility.isValueNullOrEmpty(mEmailId)) {
                edt_email_id.setText(mEmailId);
                edt_confirm_email_id.setText(mEmailId);
                edt_email_id.setEnabled(false);
                edt_confirm_email_id.setEnabled(false);
            }
        }
        if (intent.hasExtra(Constants.AUTH_TOKEN)) {
            mAuthToken = intent.getStringExtra(Constants.AUTH_TOKEN);
        }
        if (intent.hasExtra(Constants.AUTH_TYPE)) {
            mAuthType = intent.getStringExtra(Constants.AUTH_TYPE);
        }
  ...
  private void getStatesList(String mCountryID) {
   ...
   private void getVillagesList(String mMandalID) {
    ...
    private void sendAskForInvitationDetails() {
        ...
        paramMap.put("auth_token", mAuthToken);
        paramMap.put("auth_type", mAuthType);
    ...
    private boolean isValidFields() {
        ...
        if (Utility.isValueNullOrEmpty(edt_first_name.getText().toString().trim())) {
            ...
            Utility.setSnackBarEnglish(this, edt_zip_code, "Please enter Zipcode");
        ...
        if (model != null) {
            if (model.isStatus()) {
                if (model instanceof ProfessionModel) {
                    ...
                    finish();
                    Utility.showToastMessage(this, "Successfully Registered");
